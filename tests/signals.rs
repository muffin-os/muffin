...
fn signal_delivery_end_to_end() {
    ...
    // Accelerate with KVM when the host exposes it, falling back to TCG so the
    // test still runs on CI hosts without /dev/kvm. `-cpu max` (below) stays
    // valid under both, unlike `-cpu host`, which requires a hardware accelerator.
