...
/// Boots the real kernel, mounts the root ext2 filesystem, and spawns two
/// `/bin/init` instances so `tests/signals.rs` can exercise inter-process
/// signal delivery end to end. This mirrors `kernel/src/main.rs`'s boot
/// sequence but skips the graphics demo, which is irrelevant to signals.
...
unsafe extern "C" fn main() -> ! {
...

    {
        ...
        let proc2 = Process::create_from_executable(Process::root(), init_path).unwrap();
        info!("started process pid={}", proc2.pid());
    }
